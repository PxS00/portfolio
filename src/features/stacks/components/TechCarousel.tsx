        ...
        <div className="flex animate-scroll items-center gap-10 group-hover:[animation-play-state:paused]">
          {items.map((tech, idx) => (
            ...
            >
              ...
              >
                <img src={tech.iconUrl} alt={tech.name} className="h-full w-full object-contain drop-shadow-sm" />
              </motion.div>

              <div className="pointer-events-none absolute -bottom-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-zinc-800 px-3 py-1 text-xs font-medium text-white opacity-0 shadow-lg ring-1 ring-white/10 transition-opacity duration-200 group-hover/item:opacity-100">
                {tech.name}
              </div>
            </div>
          ))}
